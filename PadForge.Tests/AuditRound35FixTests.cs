...
namespace PadForge.Tests
{
    ...
    public class AuditRound35FixTests
    {
        ...
        public void DeviceList_FromAPeerWithoutTheV4Tail_StillDecodes()
        {
            ...
            // Chop the v8 tail ([magic][flags=0] for one device = 2, which of
            // the two supported-index sets were empty on purpose), the v7 tail
            // ([magic][axisMask=0][guidLen=0 (2B)] for one device = 4), the v6
            // tail ([magic][maskLen=0] = 2, the supported-button masks), the v5
            // tail ([magic][nameLen=0] = 3, the peer machine name), and then
            // the v4 tail ([magic][count] for one device = 2).
            var withoutV4 = new byte[full.Length - 13];
